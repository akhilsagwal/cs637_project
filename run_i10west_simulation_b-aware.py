...
# Predict future position of a vehicle to check for potential LOS blockage
...
# Check if an obstacle is blocking line-of-sight
...
# Calculate signal strength with blockage awareness
...
def construct_dag(vehicle_id, rsus, horizon=10, timestep=1):
    ...
    for t in range(1, horizon + 1):
        ...
        for rsu in rsus:
            distance = math.sqrt((future_pos[0] - rsu.x) ** 2 + (future_pos[1] - rsu.y) ** 2)
            if distance <= 400:  # Only consider RSUs within range
                node = (rsu.id, t)
                G.add_node(node)
                potential_datarate = 1 / (1 + distance)
                edge_weight = -potential_datarate  # Negative for shortest path calculation
    ...
    try:
        ...
        print(f"Optimal path found for vehicle {vehicle_id}: {path}")
...
def run_b_aware_simulation(config_file, rsus):
    ...
    with open('i10west_baware_simulation_log.csv', mode='w', newline='') as log_file:
        ...
        try:
            ...
            for step in range(100):  # Adjust steps as needed
...
# Load RSUs for I-10 West and run the B-AWARE simulation
rsus_i10 = load_rsus("I10West_junctions.txt")
run_b_aware_simulation("I10West.sumocfg", rsus_i10)
